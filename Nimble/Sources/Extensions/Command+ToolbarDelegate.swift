...
fileprivate extension Command {
  func createToolbarItem(_ toolbar: WorkbenchToolbar? = nil) -> ToolbarItem {
    ...
    let title: String
    if let workbench = toolbar?.nsWindow?.windowController as? Workbench {
      title = workbench.commandSates[self]?.title ?? ""
    } else {
      title = ""
    }
...
class NimbleWorkbenchCommands {
  ...
  weak var workbenchAreaGroup: CommandGroup?
  ...
  init() {
    let group = CommandGroup(name: "WorkbenchAreaGroup")
    group.palleteLable = "Workbench Areas"
    
    workbenchAreaGroup = group
    CommandManager.shared.registerGroup(group: group)
  }
  
  
  func registerCommands() {
    guard let workbenchAreaGroup = workbenchAreaGroup else { return }
